...
#include <opencv2/videoio.hpp> // Assicurati di includere la libreria video di OpenCV
...
private:
  double first_timestamp_ = -1.0; // Variabile per memorizzare il primo timestamp
  std::vector<std::vector<int>> bounding_boxes_; // Per memorizzare le bounding box

  void loadBoundingBoxes(const std::string& csv_file);
  void drawBoundingBoxes(cv::Mat& image, int normalized_timestamp);
