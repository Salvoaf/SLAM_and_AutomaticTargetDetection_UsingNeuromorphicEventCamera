...
void VioVisualizer::loadBoundingBoxes(const std::string& csv_file) {
    bounding_boxes_.clear(); // Assicurati di svuotare le bounding box precedenti
...
void VioVisualizer::drawBoundingBoxes(cv::Mat& image, int normalized_timestamp) {
    int significant_normalized_timestamp = normalized_timestamp / 10000;
    
    for (const auto& bbox : bounding_boxes_) {
        // Rimuovi le ultime 5 cifre da bbox[0]
        int significant_bbox_timestamp = bbox[0] / 10000;
        
        if (significant_bbox_timestamp == significant_normalized_timestamp) {
            int x = bbox[1];
            int y = bbox[2];
            int w = bbox[3];
            int h = bbox[4];
            if (bbox[5] == 1) {
                ...
                std::cout << "Normalized timestamp del frame: " << normalized_timestamp << std::endl;
                std::cout << "mio: " << bbox[0] << std::endl;
            }
...
{
    if (frame_indices.empty()) return;

    const std::string csv_file = "/home/salvatore/ssms_event_cameras/poses/predictions.csv";

    // Prendi l'ultimo indice di frame per visualizzare solo l'immagine a destra.
    uint32_t frame_idx = frame_indices.back();

    const Frame& frame = nframe.at(frame_idx);
    Size2u img_size = frame.pyr_->at(level).size();
    cv::Mat img_gray(img_size.height(), img_size.width(), CV_8UC1, cv::Scalar(0));

    img_gray = ImageCv8uC1(frame.pyr_->at(level)).cvMat();
    ...
    double current_timestamp = nframe.timestamp();

    // Se è la prima volta che viene chiamata, salva il timestamp iniziale
    if (first_timestamp_ < 0.0) {
        first_timestamp_ = current_timestamp;
        // Carica le bounding box dal file CSV
        loadBoundingBoxes(csv_file);
        
        // Inizializza il video writer
        std::string video_filename = "/home/salvatore/output_video.avi";
        int codec = cv::VideoWriter::fourcc('M', 'J', 'P', 'G'); // Codec per AVI
        double fps = 30.0; // Fotogrammi al secondo
        video_writer_.open(video_filename, codec, fps, img_color.size(), true);

        if (!video_writer_.isOpened()) {
            std::cerr << "Errore nell'apertura del file video per la scrittura" << std::endl;
            return;
        }
    }

    // Calcola il timestamp normalizzato usando la formula fornita
    int normalized_timestamp = int((current_timestamp - first_timestamp_) / 1e3);

    // Disegna le bounding box sull'immagine a colori
    drawBoundingBoxes(img_color, normalized_timestamp);

    // Scrivi il fotogramma al video
    video_writer_.write(img_color);
    ...
    {
        sensor_msgs::Image img_msg = ze::cvMatToImageMsg(img_color, ros::Time(current_timestamp*1e-9), "bgr8");
    ...
    const uint32_t level)
{
    ...
    if (pub_event_imgs_->getNumSubscribers() > 0)
    {
        sensor_msgs::Image img_msg = ze::cvMatToImageMsg(img_gray, ros::Time(nframe.timestamp()*1e-9), "mono8");
        pub_event_imgs_->publish(img_msg);
    }
}*/
/*
